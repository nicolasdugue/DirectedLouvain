...
static unsigned int build_map(Graph &, string, vector<unsigned long>&, vector<vector<pair<unsigned int,double> > >&, vector<vector<pair<unsigned int,double> > >&, bool, bool, bool);
...
Graph::Graph(string filename, bool reproducibility, bool renumbering, bool verbose) {
    ...
    if(extension!=".bin") {
        ...
        this->nodes = build_map(*this, filename, this->correspondance, LOUT, LIN, renumbering, reproducibility, verbose);
